...
def general_answer(question, facts, history=None):
    ...
    if groq_key:
        ...
        try:
            ...
            return (result.get('choices') or [{}])[0].get('message', {}).get('content', '').strip() or None
        except (HTTPError, URLError, TimeoutError, ValueError, OSError, AttributeError, IndexError):
            return None
    if not key:
        model = ollama_model()
        if not model:
