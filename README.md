...
Version 9 har ett gemensamt chattfält och tar bort valet ”Fri chatt”. Den kan köras publikt på Render med en serveransluten språkmodell och begränsar förfrågningar så att en enskild besökare inte förbrukar hela gratiskvoten. Vid publicering kan besökare inte starta omträning eller skriva in din datanyckel. Sevilla FC:s ordinarie arena kan visas som **trolig** hemmaarena om matchkällan inte anger spelplatsen. För andra lag prioriteras publicerad matcharena och därefter lagprofil från schemats API.
...
2. Spara `Matchorakel-uppdatera-v9.py` i Downloads och kör i PowerShell:

```powershell
py "$env:USERPROFILE\Downloads\Matchorakel-uppdatera-v9.py"
```
